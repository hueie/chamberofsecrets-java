...
JNIEXPORT void JNICALL Java_chamberofsecrets_HelloJNI_sayHello(JNIEnv * env, jobject obj, jstring jstr) {
	...
	/* File Write*/
	ofstream outputFile;
	string tmpPath = exceptExtensionPath + ".txt";
	outputFile.open(tmpPath);
	...
	cout << "Total Frame Count : "<< tot_frame_cnt << endl;
	...
	cout << "Get First Frame Image To jpg File" << endl;
	tmpPath = exceptExtensionPath + ".jpg";
	imwrite(tmpPath, curframe);
